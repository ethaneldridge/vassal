...
public class DieManager extends AbstractConfigurable {
    ...
    private Vector specialDice = new Vector();
    ...
    public DieManager() {
        ...
        GameModule.getGameModule().getPrefs().addOption(DIE_MANAGER, new StringConfigurer(SERVER_PW, "Dice Server Password"));
        GameModule.getGameModule().getPrefs().addOption(DIE_MANAGER, new BooleanConfigurer(USE_EMAIL, "Email results?"));
        GameModule.getGameModule().getPrefs().addOption(DIE_MANAGER, new StringConfigurer(PRIMARY_EMAIL, "Primary Email"));
        GameModule.getGameModule().getPrefs().addOption(DIE_MANAGER, new StringArrayConfigurer(SECONDARY_EMAIL, "Secondary Emails"));
        GameModule.getGameModule().getPrefs().addOption(DIE_MANAGER, new BooleanConfigurer(MULTI_ROLL, "Use Multi-Roll?"));
    }
    ...
    public MultiRoll getMultiRoll() {
        if (myMultiRoll == null) {
            myMultiRoll = new MultiRoll(defaultNDice, defaultNSides);
        }
        return myMultiRoll;
    }
...
*/
    ...
    private abstract class DieServer {
    ...
    private class InternetGamesDieServer extends DieServer {
        ...
        public InternetGamesDieServer() {
            ...
            maxRolls = 0;
            maxEmails = 0;
            serverURL = "http://www.internetgames.org/diceserver/dice.asp";
            passwdRequired = true;
            // password = "IG42506";
        }
        ...
        public String[] buildInternetRollString(MultiRoll toss) {
            ...
            for (int i = 0; i < 10; i++) {
                ...
                if (i < toss.rolls.length) {
                    DieRoll d = toss.rolls[i];
                    if (!s.equals("")) {
                        s +=  "&";
                    }
