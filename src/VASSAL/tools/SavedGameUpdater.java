            ...
            try {
              ...
              if (path != null && path.length > 0 && path[path.length - 1] instanceof PieceSlot) {
                ...
                if (!slot.getPiece().getType().equals(p.getType())) {
                  Replace r = new Replace(Replace.ID + "Replace;R;dummy;;0;0;true", p) {
                    protected GamePiece createMarker() {
                      GamePiece replacement = PieceCloner.getInstance().clonePiece(slot.getPiece());
                      if (matchRotation) {
                        matchTraits(getInner(), replacement);
                      }
                      return replacement;
                    }
                  };
                  r.setProperty(VASSAL.counters.Properties.OUTER, p);
                  p.setProperty(VASSAL.counters.Properties.OUTER, null);
                  r.myKeyEvent(KeyStroke.getKeyStroke('R', InputEvent.CTRL_MASK));
                }
              }
            }
