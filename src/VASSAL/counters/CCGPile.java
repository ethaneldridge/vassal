...
import VASSAL.counters.CCGDeck;
...
public class CCGPile extends SetupStack {
  protected CCGDeck dummy = new CCGDeck(); // Used for storing type information
  private boolean reshufflable;
  private CCGDeck myDeck;
  ...
  private static UniqueIdManager idMgr = new UniqueIdManager("DonDeck");
  ...
  /**
   * Return the CCGPile instance with the matching id or name
   ...
   */
  public static CCGPile findCCGPile(String id) {
    return (CCGPile) idMgr.findInstance(id);
  }
  ...
  public static final String REVERSIBLE = "Xreversible";
  ...
  public static final String DECK_NAME = "DonDeckName";
  ...
  public static class AssignedDeckPrompt extends StringEnum {
    ...
    public String[] getValidValues(AutoConfigurable target) {
      ...
      for (Enumeration e = GameModule.getGameModule().getComponents(Map.class); e.hasMoreElements();) {
        m = (Map) e.nextElement();
        if (m != null) {
          for (Enumeration e1 = m.getComponents(CCGPile.class); e1.hasMoreElements();) {
            dp = (CCGPile) e1.nextElement();
  ...
  protected Stack initializeContents() {
    ...
    myDeck = new CCGDeck(getDeckType());
    
    for (Enumeration e = s.getPieces(); e.hasMoreElements();) {
      myDeck.add((GamePiece) e.nextElement());
    }
    myDeck.setFaceDown(!CCGDeck.NEVER.equals(dummy.getFaceDownOption()));
    return myDeck;
  }
