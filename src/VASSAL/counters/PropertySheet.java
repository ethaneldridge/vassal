...
public class PropertySheet extends Decorator implements EditablePiece {
  ...
  java.util.Vector m_fields; //TODO: dispose this if frame destroyed (is this possible?)
  ...
  protected KeyCommand[] myGetKeyCommands() {
    if (launch == null) {
      launch = new KeyCommand(menuName, KeyStroke.getKeyStroke(launchKey, java.awt.event.InputEvent.CTRL_MASK), Decorator.getOutermost(this));
    }
  ...
  private void updateStateFromFields() {
    ...
    if (state != encoder.getValue()) {

      state = encoder.getValue();
  ...
  private void updateFieldsFromState() {
    ...
    if (frame != null) {
      ...
      for (int iField = 0; stateDecoder.hasMoreTokens(); ++iField) {
  ...
  public Command myKeyEvent(KeyStroke stroke) {
    ...
    if (launch.matches(stroke)) {
      if (frame == null) {
        ...
        m_fields = new Vector();
        // We can apply a custom icon for this window with this code:
        //  JFrame DummyFrame = new JFrame();
        //  final Image icon =
        //    Toolkit.getDefaultToolkit().getImage( "E:/eclipse/workspace/LayeredPaneDemo/images/dukeWaveRed.gif" );
  ...
  public VASSAL.build.module.documentation.HelpFile getHelpFile() {
    ...
    try {
      return new HelpFile(null, new File(dir, "PropertySheet.htm"));
    }
...
//      int numRows = propertyTable.getRowCount();
  ...
  class TickPanel extends JPanel implements ActionListener, FocusListener, DocumentListener {
    ...
    public TickPanel(String value, int type) {
      ...
      //Debug.trace("TickPanel( " + value + ", " + type + ")");
      set(value);
