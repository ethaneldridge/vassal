...
public class PBConcealable extends Obscurable implements EditablePiece {
  ...
  public PBConcealable() {
    this(ID + "C;Qmark58;ge", null);
  }

  public PBConcealable(String type, GamePiece inner) {
    super(type, inner);
    mySetType(type);
    hideCommand = "Conceal";
  }
    ...
    if (commands[0].matches(stroke)
      ...
      if (getParent() != null) {
        for (int i = getParent().indexOf(outer),j = getParent().getPieceCount(); i < j; ++i) {
          PBConcealment conceal = (PBConcealment) Decorator.getDecorator(getParent().getPieceAt(i), PBConcealment.class);
          if (conceal != null
      ...
      if (!concealmentExists) {
        ...
        PBConcealment conceal = (PBConcealment) Decorator.getDecorator(concealOuter, PBConcealment.class);
        c.append
  ...
  public Command adjustConcealment() {
    if (isMaskableBy(GameModule.getUserId())) {
      ...
      if (getParent() != null) {
        for (int i = getParent().indexOf(outer),j = getParent().getPieceCount(); i < j; ++i) {
          PBConcealment p = (PBConcealment) Decorator.getDecorator(getParent().getPieceAt(i), PBConcealment.class);
  ...
  public static Command adjustConcealment(Stack s) {
    ...
    for (int i = 0,j = s.getPieceCount(); i < j; ++i) {
      PBConcealable p = (PBConcealable) Decorator.getDecorator(s.getPieceAt(i), PBConcealable.class);
      if (p != null) {
  ...
  public GamePiece createConcealment() {
    ...
    p = new Delete(Delete.ID + "Delete;X", p);
    p = new PBConcealment(PBConcealment.ID + GameModule.getUserId() + ";" + nation, p);
