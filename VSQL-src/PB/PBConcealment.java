 ...
 */
public class PBConcealment extends Decorator implements EditablePiece {
  ...
  public PBConcealment() {
    this(ID, null);
  }

  public PBConcealment(String type, GamePiece p) {
    setInner(p);
    mySetType(type);
  }
    ...
    if (parent != null) {
      ...
      if (newIndex > lastIndex) {
        ...
        for (int i = lastIndex; i > newIndex; --i) {
          ...
          if (Decorator.getDecorator(child, PBConcealment.class) != null) {
            break;
          }
  ...
  public boolean canConceal(GamePiece p) {
    PBConcealable c = (PBConcealable) Decorator.getDecorator(p, PBConcealable.class);
    if (c == null
